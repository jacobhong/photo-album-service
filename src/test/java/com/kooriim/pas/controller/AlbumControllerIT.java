...
import com.kooriim.pas.SecurityConfig;
...
//    final var photos = photoRepository.getPhotosByAlbumId(album.getBody().getId()).get();
//    assertEquals(1, photos.size());
////    assertEquals(1, photos.get(0).getId());
//    assertEquals("anonymousUser", photos.get(0).getGoogleId());
//  }
  ...
  public void testGetAlbumsByGoogleId() {
    restTemplate.exchange(uri(), HttpMethod.POST, httpEntityWithAlbum(), Album.class);
    final var albums = restTemplate.exchange(uri(), HttpMethod.GET, httpEntity(), Album[].class);
